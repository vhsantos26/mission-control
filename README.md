...
- **v0.5.0** — **Codex support**: novo `SessionSource` interface em `src/sources/`, com `claude_code` e `codex` paralelos. Codex usa semântica *last-wins* nos token totals (são cumulativos no JSONL, somar multiplicaria por N). **OpenAI pricing** (gpt-5/-codex, gpt-4o, o1, o3) com longest-prefix resolve — `gpt-5.3-codex-2026-01` cai certo em `gpt-5.3-codex`. `cache_create` fixo em 0 para OpenAI (não cobram criação separada); `cache_read` = 50% do input. **Top tools chart** em Overview, agregando `tool_use` blocks dos prompts (nova tabela `tool_uses`, endpoint `/api/by-tool`). Sessions agora ordenam por `started_at DESC` (mtime fica como tiebreaker) — resolve confusão de datas saltando na coluna 'Início' enquanto sessões ativas continuavam atualizando mtime. Drill-down de prompts também ordenado DESC pra casar com a tabela mãe. Refactor de logging: stdlib `logging` em `src/log.py`, `ScanResult` dataclass, scans em estado estável ficam silenciosos (só logam quando há `+new` ou `-cleaned`). Bug fix: prompts agora fazem UPDATE no conflito (era `INSERT OR IGNORE`, perdia atualizações de tokens em re-scans). Bug fix: coluna "Sessões" da aba Projects agora popula (estava `—`).
